...
// El escenario sale del MISMO curso, preguntando por el metro central de la
// franja: asi el paisaje no puede quedarse en la terminal anterior cuando el
// portico ya cambio. Antes bastaba con dividir la distancia entre un largo de
// zona fijo; ahora las terminales salen sorteadas y cada una mide lo suyo, o
// sea que la unica forma de saber que toca es preguntarselo al curso.
const themeFor = (k) => zoneKeyAt(k * L)

// Color de cada terminal, para el cartel que cuelga del portico de patio: el
// escenario y el HUD dicen "estas en la intermodal" con el mismo amarillo.
const ACCENT = Object.fromEntries(ZONES.map((z) => [z.key, z.accent]))
...
function buildProps(theme, seed) {
  ...
  const acento = ACCENT[theme] || COLORS.sky
  const out = []
  ...
  /* ---------- EL FONDO: LO QUE CIERRA EL CIELO ----------

    Un puerto de verdad no tiene el horizonte libre. Detras del patio hay naves,
    silos, la linea de gruas del muelle de al lado y, mas atras, la ciudad. Sin
    nada de eso el corredor iba por una pista con medio cuadro de cielo naranja:
    por bonito que sea el atardecer, el cielo no da velocidad -- no se mueve --
    ni dice donde estas. Lo que llena esa mitad de cuadro es esto.

    Tres reglas, y las tres importan:

      1. EMPIEZA LEJOS. Nada de aqui puede taparle el carril a nadie ni meterse
         entre la camara y el corredor: la fila de cerca arranca en x=+-19 y la
         de lejos en +-40. Es la misma leccion que ya estaba escrita para los
         edificios de la intermodal.

      2. LA FILA DE LEJOS ES CONTINUA. Cada pieza mide lo que la franja (30 m) y
         va centrada en ella, asi que encaja con la de la franja siguiente y no
         aparecen huecos de cielo al reciclarse. Lo que cambia de una franja a
         otra es la ALTURA, que es justo lo que dibuja un perfil de ciudad.

      3. VA PELADA (flat): sin mapas, sin sombras y sin canto matado. Son las
         piezas que mas pixeles ocupan del escenario entero y a esa distancia no
         se ve el grano de nada -- se ve la silueta. Ver SegmentMesh y merge.js.
  */
  const HAZE = ['#5f6878', '#535c6b', '#6b7482', '#4c5563', '#79828f']
  const haze = () => HAZE[Math.floor(rnd() * HAZE.length)]
  // TODO EL FONDO COMPARTE ACABADO, y no es pereza: la fusion agrupa por
  // material (ver merge.js), asi que cada acabado distinto es una malla mas por
  // franja. Aqui no hace falta ninguno: lo que distingue un edificio de otro a
  // sesenta metros es la silueta y el color, y el color va en los vertices.
  const FONDO = { flat: true, roughness: 1, metalness: 0.05 }
  // Lo que va de cuarenta metros para alla no se ilumina: se pinta (ver
  // SegmentMesh). La fila de cerca si, porque todavia es recinto y se le nota.
  const SILUETA = { silueta: true }
  // Y una sola luz para todo lo que se enciende ahi detras -- ventanas y
  // balizas -- por lo mismo: el emisivo es del material, asi que dos colores de
  // luz son dos mallas.
  const LUZ_FONDO = { emissive: '#ffd08a', emissiveIntensity: 1.8 }
  const lejos = (x, y, z, size, color, extra = {}) => box([x, y, z], size, color, { ...FONDO, ...extra })

  // Fila de siluetas de un lado, de `desde` a `hasta` metros del eje.
  const fila = (sx, desde, hasta, hmin, hmax, base = 0, extra) => {
    let x = desde
    while (x < hasta) {
      const w = Math.min(6 + rnd() * 14, hasta - x)
      const h = hmin + rnd() * (hmax - hmin)
      lejos(sx * (x + w / 2), base + h / 2, 0, [w, h, L], haze(), extra)
      // huecos estrechos: por ellos se cuela el naranja del cielo y es lo que
      // hace que la fila se lea como edificios sueltos y no como un muro
      x += w + rnd() * 3
    }
  }

  // Grua de portico vista de lejos. Es la silueta que dice PUERTO a doscientos
  // metros y la que rompe la linea recta de los tejados: sin una o dos de estas
  // por franja, el fondo se lee como un poligono industrial cualquiera.
  const gruaLejos = (x, z, h) => {
    const c = haze()
    const S = SILUETA
    for (const dx of [-5.5, 5.5]) {
      lejos(x + dx, h / 2, z - 3, [1.2, h, 1.2], c, S)
      lejos(x + dx, h / 2, z + 3, [1.2, h, 1.2], c, S)
    }
    lejos(x, h + 0.9, z, [14, 1.8, 7.5], c, S)
    // pluma levantada, que es como esperan las gruas de muelle
    lejos(x - 13, h + 6.5, z, [22, 1.2, 2.4], c, { ...S, rot: [0, 0, 0.5] })
    lejos(x + 8, h + 4.2, z, [10, 1.1, 2.2], c, { ...S, rot: [0, 0, -0.45] })
  }

  // Torre de alumbrado del patio de al lado, con su baliza roja. Sube por encima
  // de todo lo demas: es lo unico que llega a la parte alta del cuadro.
  const torreLejos = (x, z, h) => {
    lejos(x, h / 2, z, [1.1, h, 1.1], '#4c5563', SILUETA)
    lejos(x, h + 0.6, z, [4.2, 1.2, 1.6], '#5f6878', SILUETA)
    sph([x, h + 1.8, z], [0.3, 8, 8], '#ffb27a', LUZ_FONDO)
  }

  // El telon entero. Lo llama cada terminal al final con su propio sabor.
  //
  // `desde` es lo que de verdad hay que pensar en cada una: es la distancia a la
  // que empieza el perfil, y por tanto lo cerca que se siente. En un patio va a
  // cuarenta metros, pegado detras de la operacion; cruzando el agua tiene que
  // irse mucho mas lejos, porque ahi no es el fondo del recinto sino la OTRA
  // ORILLA, y a cuarenta metros se leia como un canal de paredes grises.
  const telon = ({
    cerca = [7, 13],
    lejano = [12, 30],
    desde = 40,
    ...
    for (const sx of [-1, 1]) {
      // Orilla del fondo: la barra de tierra sobre la que se apoya todo. Sin
      // ella los edificios salian del agua, porque pasados los treinta metros
      // de patio lo unico que hay es el disco de mar.
      if (orilla) lejos(sx * (hasta + 22), 0.4, 0, [(hasta - desde) * 1.6 + 30, 2.4, L], '#3b4452', SILUETA)
      if (cerca) fila(sx, 19, 33, cerca[0], cerca[1])
      fila(sx, desde, hasta, lejano[0], lejano[1], 1.2, SILUETA)
      // Segunda linea, mas alta y mas suelta: da profundidad al perfil. Solo en
      // el nivel bueno: es una fila entera de cajas dibujada DETRAS de otra, o
      // sea puro pixel repetido, que es justo lo que ahoga a una integrada.
      if (QUALITY.fx && rnd() < 0.75) {
        const x = hasta + 4 + rnd() * 26
        lejos(sx * x, 1.2 + (lejano[1] * 0.55) / 2, 0, [10 + rnd() * 18, lejano[1] * 0.55 + rnd() * 12, L], haze(), SILUETA)
      }
      // VENTANAS ENCENDIDAS en la cara que da al recinto. Son ocho puntos de luz
      // por franja y hacen mas por que el fondo parezca habitado que otras
      // veinte cajas: a esta hora, en un puerto, lo que se ve de lejos son las
      // luces, no los edificios.
      for (let i = 0; i < 4; i++) {
        sph([sx * (desde + 0.6), 3 + rnd() * (lejano[1] * 0.7), (rnd() - 0.5) * L * 0.9], [0.22, 6, 6], '#ffd98a', LUZ_FONDO)
      }
    }
    for (let i = 0; i < gruas; i++) {
      const sx = rnd() < 0.5 ? -1 : 1
      gruaLejos(sx * (desde + 4 + rnd() * (hasta - desde) * 0.5), (rnd() - 0.5) * L * 0.7, 16 + rnd() * 8)
    }
    if (torres && seed % 2 === 0) {
      torreLejos((rnd() < 0.5 ? -1 : 1) * (desde - 4 + rnd() * 30), (rnd() - 0.5) * L, 26 + rnd() * 10)
    }
  }

  /* ---------- POR ENCIMA DEL CORREDOR ----------

    Lo otro que cierra el cielo, y el que de verdad se siente: algo que pasa por
    encima de la cabeza. Un runner con el cielo abierto de punta a punta no tiene
    ritmo; lo que da ritmo es cruzar por debajo de una cosa cada pocos segundos.

    ALTURA: la viga mas baja va a 7.8 m. Por debajo tiene que pasar TODO, y lo
    mas alto que llega el jugador es colgado del gancho de la grua, a 4.8 m mas
    su propia estatura. Con menos de siete se le metia por la cabeza a alguien.
    Las patas van en x=+-7.6, fuera de los tres carriles y de sus cantos.
  */
  // Mismo criterio que el fondo: un acabado para toda la estructura y la luz
  // compartida con el. Un portico llego a pedir seis mallas el solo.
  const ACERO = { metalness: 0.35, roughness: 0.6 }
  const dosel = (z, tipo, tinte = '#8d99a6', lado = 6.4) => {
    const Y = 7.8
    // La viga llega de pata a pata. `lado` lo pone cada terminal, porque lo que
    // hay pegado al corredor cambia: en la de contenedores las pilas empiezan en
    // x=5.8 y las patas tienen que entrar por dentro; en la intermodal hay dos
    // espuelas con vagones y tienen que salir por fuera.
    const W = lado * 2 + 2.4
    for (const sx of [-1, 1]) {
      box([sx * lado, Y / 2, z], [0.72, Y, 0.72], tinte, ACERO)
      box([sx * lado, 0.35, z], [1.5, 0.7, 1.5], '#55606b', ACERO)
      box([sx * (lado - 0.45), Y - 1.4, z], [1.6, 0.5, 0.5], tinte, { ...ACERO, rot: [0, 0, 0.9] })
    }
    if (tipo === 'tubos') {
      // rack de tuberias: lo que cruza un patio de graneles de un lado a otro
      box([0, Y + 0.35, z], [W, 0.4, 2.8], '#6f7b86', ACERO)
      for (const dz of [-0.9, 0, 0.9]) {
        cyl([0, Y + 0.95, z + dz], [0.3, 0.3, W - 0.6, 10], dz === 0 ? '#c2762c' : '#9aa4ae', {
          ...ACERO,
          rot: [0, 0, Math.PI / 2],
        })
      }
    } else if (tipo === 'cinta') {
      // cinta transportadora encapsulada cruzando por lo alto
      box([0, Y + 0.9, z], [W + 0.4, 1.7, 3.2], '#b8bdc2', ACERO)
      box([0, Y + 1.85, z], [W + 0.4, 0.25, 3.4], '#7d858d', ACERO)
      for (let i = -6; i <= 6; i += 3) sph([i, Y + 0.3, z + 1.5], [0.11, 8, 8], '#ffe9a0', LUZ_FONDO)
    } else if (tipo === 'catenaria') {
      // portico de catenaria: dos vigas y los brazos que sostienen el hilo
      for (const dz of [-1.3, 1.3]) box([0, Y + 0.3, z + dz], [W, 0.3, 0.3], tinte, ACERO)
      box([0, Y + 0.9, z], [W, 0.22, 0.22], tinte, ACERO)
      for (const sx of [-1, 1]) box([sx * 3.8, Y - 0.35, z], [0.16, 0.9, 0.16], tinte, ACERO)
      box([0, Y - 0.75, z], [11, 0.09, 0.09], '#3c454e', ACERO)
    } else {
      // cartel de patio: el letrero grande con el color de la terminal
      box([0, Y + 0.45, z], [W, 0.45, 1.0], tinte, ACERO)
      box([0, Y + 1.5, z], [9.5, 1.7, 0.35], '#16202c', ACERO)
      box([0, Y + 1.5, z - 0.22], [8.6, 1.1, 0.12], acento, {
        emissive: acento,
        emissiveIntensity: 0.55,
        roughness: 0.4,
      })
      for (const sx of [-1, 1]) sph([sx * 5.4, Y + 1.5, z - 0.3], [0.14, 8, 8], '#ffe9a0', LUZ_FONDO)
    }
  }
  ...
  if (theme === 'tum') {
    ...
    const SHIP_X = -13.4
    // Popa y proa del buque de ESTE dique. Ya no son constantes del modulo: en
    // una carrera infinita se cruzan varios astilleros y cada uno tiene el suyo
    // en otro metro, asi que el rango se le pregunta al curso.
    const rango = shipRange(dSeg) || { from: 0, to: 0 }
    const SHIP_FROM = rango.from
    const SHIP_TO = rango.to
    const SHIP_LEN = Math.max(1, SHIP_TO - SHIP_FROM)
  ...
  /* ---------- Cierre de cielo, terminal por terminal ----------

    Va al final y lo lleva TODA franja, sea del tema que sea: el fondo no
    depende de en que terminal estas -- el cielo hay que cerrarlo siempre --, y
    lo unico que cambia es de que esta hecho el perfil y que cruza por arriba.

    Los porticos van pegados a los CANTOS de la franja (z = +-13) y no al
    centro, que es donde cada terminal planta su grua de patio: asi no se
    encajan una dentro de otra al sortearse las dos en la misma franja.
  */
  const canto = L / 2 - 2.5
  if (theme === 'astillero') {
    // el dique ya tiene sus dos macizos de muelle a los lados y un buque de
    // catorce metros tapando babor: aqui solo falta lo que asoma POR ENCIMA de
    // todo eso, o sea la fila de lejos y las gruas del muelle de al lado
    telon({ cerca: false, lejano: [14, 30], gruas: 2 })
  } else if (theme === 'crucero') {
    const etapa = cruStage(seed * L) || 'muelle'
    // Perfil bajo y sin fila de cerca en el agua: ahi el fondo es la otra
    // orilla de la bahia, no un patio. En el muelle si hay recinto.
    telon({
      cerca: etapa === 'muelle' ? [6, 11] : false,
      lejano: [8, 20],
      desde: etapa === 'muelle' ? 46 : 64,
      hasta: etapa === 'muelle' ? 92 : 112,
      gruas: 1,
    })
    // pasarela de embarque cruzando el muelle: cerrada y blanca, como las de
    // verdad, y solo donde hay muelle bajo los pies
    if (etapa === 'muelle' && seed % 2 === 0) dosel(canto, 'cinta', '#d9e0e6', 6.6)
  } else if (theme === 'intermodal') {
    telon({ cerca: [7, 13], lejano: [12, 26], gruas: 1 })
    // DOS PORTICOS DE CATENARIA POR FRANJA, uno en cada canto. Es la pieza que
    // mas se nota de todas estas: pasar por debajo de algo cada quince metros
    // es lo que le da ritmo a un tramo recto, y en una terminal de ferrocarril
    // es ademas lo que de verdad hay ahi arriba.
    dosel(-canto, 'catenaria', '#7f8a95', 9.8)
    dosel(canto, 'catenaria', '#7f8a95', 9.8)
  } else if (theme === 'tec') {
    telon({ cerca: [9, 16], lejano: [14, 32], gruas: 2 })
    // las patas entran POR DENTRO de las pilas, que empiezan en x=5.8
    if (seed % 3 === 0) dosel(canto, 'cartel', '#8d99a6', 5.0)
    else if (seed % 3 === 1) dosel(-canto, 'tubos', '#8d99a6', 5.0)
  } else {
    telon({ cerca: [8, 14], lejano: [12, 26], gruas: 1 })
    // en la de graneles lo que cruza son tuberias y cintas, que es como se
    // mueve el granel de verdad de un lado del patio al otro
    if (seed % 3 === 0) dosel(canto, 'tubos', '#9aa4ae', 6.6)
    else if (seed % 3 === 1) dosel(-canto, 'cinta', '#b8bdc2', 6.6)
  }
...
function SegmentMesh({ part, maps }) {
  ...
  // EL FONDO VA PELADO: sin mapa de color, de relieve ni de rugosidad. Son las
  // piezas que mas pantalla ocupan -- naves y gruas de veinte metros al otro
  // lado del patio -- y tres lecturas de textura por pixel en media pantalla es
  // exactamente lo que ahoga a una grafica integrada. Ademas es lo correcto:
  // a esa distancia no se ve el grano de nada, se ve la silueta.
  const plano = !!p.flat
  // SILUETA: el perfil lejano no se ILUMINA, se pinta. Es la mitad de la
  // pantalla en cuanto se cierra el cielo, y hasta sin mapas seguia pagando
  // sombras, dos direccionales, hemisferico y reflejo de entorno por cada uno
  // de esos pixeles. A sesenta metros y con la bruma encima, todo eso no se ve:
  // lo que se ve es la silueta contra el atardecer. Sigue entrando en la niebla,
  // que es lo que la funde con el horizonte pintado del domo.
  const silueta = !!p.silueta
  return (
    ...
    <mesh
      ...
      // el fondo tampoco entra en el juego de sombras: esta fuera de la caja
      // del mapa (34 m alrededor del corredor) y nada de lo que hay dentro
      // recibiria su sombra
      castShadow={QUALITY.shadows && !lamp && !plano && !silueta}
      receiveShadow={QUALITY.shadows && !plano && !silueta}
      userData={DONE}
    >
      {silueta ? (
        ...
        <meshStandardMaterial
          vertexColors
          map={lamp || plano ? null : map}
          emissive={p.emissive || '#000000'}
          emissiveIntensity={p.emissiveIntensity || 0}
          metalness={p.metalness || 0.1}
          roughness={p.roughness ?? 0.85}
          normalMap={lamp || plano || !QUALITY.normalMap ? null : d.normal}
          roughnessMap={lamp || plano || !QUALITY.roughMap ? null : d.rough}
          envMapIntensity={plano ? 0.6 : 1.15}
        />
      )}
    </mesh>
  )
