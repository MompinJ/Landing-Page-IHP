...
import { zoneIndexAt, deckAt, seaLevelAt, ensureCourse } from '../course'
...
import { BASE_SPEED, MAX_SPEED, SPEED_RAMP, PLAYER_Z, COLORS, LANES, JUMP_BUFFER } from '../constants'
...
function Loop() {
  ...
  useFrame((_, dt0) => {
    ...
    scroll.s += scrollSpeed(phase) * dt
    // EL CURSO SE ESCRIBE POR DELANTE. Es lo unico que hace falta para que sea
    // infinito: mientras el corredor avanza, aqui se van anadiendo terminales
    // enteras con margen de sobra sobre la niebla (ver ensureCourse).
    ensureCourse(scroll.s)
    // el oleaje de todas las laminas de agua avanza aqui, con un solo reloj
    tickWater(dt)

    // FRENADO DEL CHOQUE. Con una sola vida la carrera se acaba en el primer
    // golpe, pero no se corta en el fotograma del impacto: el mundo se para en
    // poco mas de un segundo y ahi se ve contra que fue. Cortar a negro de
    // golpe deja al jugador del stand preguntando que paso.
    if (phase === 'crashed') {
      runtime.dead += dt
      runtime.speed = Math.max(0, runtime.speed - dt * 26)
    ...
    if (phase !== 'playing') {
      ...
      // el techo de un camion. Pisar aqui su suelo lo dejaba caido al asfalto
      // al pausar, y al reanudar aparecia abajo.
      if (phase !== 'paused') runtime.deck = deckAt(scroll.s)
      return
    }
    runtime.elapsed += dt
    // Gracia despues de partirse el casco: la pieza contra la que se choco
    // sigue ahi y sin esto cobraria el casco y la vida en dos cuadros seguidos.
    if (runtime.invuln > 0) runtime.invuln = Math.max(0, runtime.invuln - dt)
    // La velocidad sale de los METROS y no del reloj: sin final, el reloj no
    // dice nada, y lo que tiene que costar mas es haber llegado lejos.
    runtime.speed = BASE_SPEED + (MAX_SPEED - BASE_SPEED) * Math.min(1, runtime.distance / SPEED_RAMP)
    runtime.distance += runtime.speed * dt
    useGame.getState().syncHud(Math.floor(runtime.distance))
...
function CameraRig() {
  ...
  useFrame((state, dt0) => {
    ...
    //
    // COLGADO DE LA GRUA SUBE POCO, Y ES A PROPOSITO. La primera version la
    // subia casi tanto como al corredor y quedaba a mas de ocho metros: ahi
    // arriba la camara se mete DENTRO del patio -- pilas de contenedores de tres
    // alturas, vigas y spreaders colgados de las gruas -- y se le cuela por
    // delante una pieza de decorado que tapa media pantalla. Lo que sube en el
    // izado es el ENCUADRE (ver el lookAt de abajo), no el ojo: el corredor se
    // va arriba del cuadro, que es justo lo que hace que se sienta alto.
    const izado = runtime.flying ? 0.35 : 0.28
    cam.position.y = 4.3 + ground.current + runtime.y * izado - dip.current * 0.85
    ...
    cam.lookAt(
      runtime.x * 0.4,
      1.3 + ground.current + (runtime.flying ? runtime.y * 0.55 : 0) - dip.current * 0.35,
      PLAYER_Z - 9
    )
...
export function Game() {
  ...
  return (
    ...
    >
      ...
      <RecordLine />
      <Items />
      {/* Detras de CameraRig a proposito: las rayas de velocidad se plantan en
          la camara cada cuadro, asi que tienen que leerla ya movida. */}
      <SpeedLines />
      <Fx />
      {PERF && <Perf />}
    </Canvas>
