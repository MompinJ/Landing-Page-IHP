...
const Obstacle = memo(function Obstacle({ ob, maps }) {
  ...
  useFrame((_, dt0) => {
    ...
    // Colgado del gancho de la grua no le pega nada: va por encima de todo lo
    // que hay en pista, y ese es justo el trato del izado. Se comprueba aqui y
    // no por la altura porque el vuelo empieza y acaba a ras de suelo, y en esos
    // metros el corredor esta pasando POR DENTRO de lo que sea que haya.
    if (phase === 'playing' && !runtime.flying && !st.hit && sameLevel && Math.abs(z - PLAYER_Z) < half) {
