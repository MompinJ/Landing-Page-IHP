...
export function Scaffolds() {
  ...
  useFrame(() => {
    let [a, b] = cur.current
    // Los cursores solo avanzan, pero pueden VOLVER a cero: el curso se rehace
    // entero en cada carrera, asi que al reiniciar la lista es otra y la ventana
    // se habria quedado apuntando mas alla de su final.
  ...
  // Solo los andamios: los camiones portacontenedor son plataformas de la misma
  // lista y con la misma mecanica, pero se dibujan en Rigs.
  return PLATFORMS.slice(win[0], win[1])
    .filter((p) => p.kind === 'scaf')
    .map((p) => <Scaffold key={`${p.kind}-${p.d0}`} p={p} />)
}
