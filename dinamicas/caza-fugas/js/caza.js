...
(() => {
  ...
  const missClick = (x, y) => {
    ...
    hudStatus.textContent = 'AHI NO HAY FUGA (-25)';
    updateHud();
  };
  ...
  const endRound = (complete) => {
    ...
    const rank = found.size === TOTAL && score >= TOTAL * HIT ? 'Guardian del oceano.' :
                 pct >= 80 ? 'Auditor verde.' :
                 pct >= 50 ? 'Eco-observador en formacion.' : 'El planeta pide refuerzos.';

    $('finale-kicker').textContent = complete ? 'TERMINAL LIMPIA' : 'SE ACABO EL TIEMPO';
    $('finale-title').textContent = rank;
    $('finale-score').textContent = `${Math.max(0, score)} PTS`;
    $('finale-detail').textContent =
      `${found.size}/${TOTAL} FUGAS HALLADAS` +
  ...
  const reset = () => {
    ...
    finale.hidden = true;
    toast.hidden = true;
    hudStatus.textContent = 'ENCUENTRA LAS FUGAS Y DERROCHES: CLIC SOBRE CADA UNO';
    updateHud();
  };
